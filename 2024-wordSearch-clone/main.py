...
async def save_game_info(game_info: GameInfo):
  ...
  try:
      ...
      cursor.execute("INSERT INTO games (title, description ,create_user_id) VALUES (%s, %s ,%s)", (game_info.title, game_info.description , user_id))
      db.commit()
      # 새로 생성된 게임의 id 가져오기
      game_id = cursor.lastrowid
      # 게임의 단어들을 words 테이블에 저장
      for word in game_info.words:
          cursor.execute("INSERT INTO words (word, game_id) VALUES (%s, %s)", (word, game_id))
      db.commit()
      game_url = f"http://localhost:5173/#/game/{game_id}"
      return JSONResponse(content={"message": "Game information saved successfully", "game_url": game_url},status_code=200)
  except mysql.connector.Error as err:
      db.rollback()
      return JSONResponse(content={"error": str(err)},status_code=500)
...
async def update_game_info_for_url(update_game_info : UpdateGameInfo):
  cursor = db.cursor()
  try:
    cursor.execute("UPDATE games SET url = %s WHERE id = %s", (update_game_info.url , update_game_info.gameId))
    db.commit()
...
async def get_game_info(gameId: int):
    ...
    try:
        ...
        if game_info:
            return JSONResponse(content={"message": "Game information is available", "game_info": game_info}, status_code=200)
        else:
            return JSONResponse(content={"message": "Game information is not available"}, status_code=404)
    except mysql.connector.Error as err:
        return JSONResponse(content={"error": str(err)}, status_code=500)
